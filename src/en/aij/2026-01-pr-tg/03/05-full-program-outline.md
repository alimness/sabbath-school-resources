---
title: Full Program Outline
markdownTitle: '^[Full Program Outline]({"style": {"text": {"color": "#F4793B"}}})'
subtitle: Week 3
---

{"style": {"text": {"color": "#F4793B"}}}
**Big Idea:** God gave us the Sabbath, a very special day to spend with Him.

{"style": {"block": {"backgroundColor": "#E4E4E3", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#6D6E6D", "size": "sm"}}}
**EARLY ARRIVALS** (10 MINUTES BEFORE SABBATH SCHOOL BEGINS)

**^[LISTEN:]({"style": {"text": {"color":"#4d4d4f"}}})** Play the song **“When God Speaks”** or worship songs of your choice quietly in the background.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/when-god-speaks.mp3] “When God Speaks” {"credits":[{"key":"Writer","value":"Braedan Entermann"},{"key":"Singers","value":"Alise Entermann, Braedan Entermann"},{"key":"Pianist","value":"Braedan Entermann"},{"key":"Guitarist","value":"Braedan Entermann"},{"key":"Producer","value":"Rhys Zacher"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

**^[CONNECT:]({"style": {"text": {"color":"#4d4d4f"}}})** Greet the children warmly as they arrive. Introduce church members who have brought in different nature treasures.

**^[DO:]({"style": {"text": {"color":"#4d4d4f"}}})** Look at the natural objects together (e.g., gems or rocks, amazing feather, speckled egg, beetle shell). Allow children to ask questions and church members can share and connect with the children.

{"style": {"block": {"backgroundColor": "#F9E7E3", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#6D6E6D", "size": "sm"}}}
**OPENING ACTIVITIES** (10 MINUTES)

**^[WELCOME:]({"style": {"text": {"color":"#d34d5e"}}})** Bring the children together and welcome them as a group.

**^[SING:]({"style": {"text": {"color":"#d34d5e"}}})** Sing **“God Is Our Creator”** and “Be Still, Be Silent.”

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/god-is-our-creator.mp3] “God Is Our Creator” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"David Williams, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Cello","value":"Jared Chung"},{"key":"Guitarist","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

**^[PRAY:]({"style": {"text": {"color":"#d34d5e"}}})** Invite two children to pray, thanking God for the Sabbath.

**^[BIBLE FOCUS:]({"style": {"text": {"color":"#d34d5e"}}})** Listen to and learn **“The Books of the Bible”** song. Share the **Bible Fast Fact:** Moses wrote the first five books of the Bible. **Ask:** Who can name them in order? (Genesis; Exodus; Leviticus; Numbers; Deuteronomy.) These books are called the Torah or the Pentateuch.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/books-of-the-bible.mp3] “The Books of the Bible” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Johanna McKay, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Guitarist","value":"David Williams"},{"key":"Bass","value":"David Williams"},{"key":"Percussion","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2025 General Conference Corporation of Seventh-day Adventists®"}

**^[GIVE:]({"style": {"text": {"color":"#d34d5e"}}})** Have a child collect the offering as you all sing **“Thank You, Jesus.”** Later, after the money has been counted by an adult, invite a child to color the next section of your **Offering Chart** to show how much has been collected this quarter. Then share this week’s **Mission Fast Fact** about this quarter’s mission project. Say a short prayer for God to bless and take the offering to where it’s most needed.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/thank-you-jesus.mp3] “Thank You, Jesus” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"David Williams, Johanna McKay, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Guitarist","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

**^[SING:]({"style": {"text": {"color":"#d34d5e"}}})** Warmly welcome any visitor and sing the **“Visitor Song.”** Sing **“Happy Birthday”** to a child who has had a birthday this week, and pray for God to bless them.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/visitor-song.mp3] “Visitor Song” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singer","value":"Johanna McKay"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/birthday-song.mp3] “Happy Birthday” {"credits":[{"key":"Writer","value":"Janet Sage"},{"key":"Singer","value":"David Williams"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Guitarist","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2001 North American Division Corporation of Seventh-day Adventists. From Little Voices Praise Him, #39. Used by permission"}

{"style": {"block": {"backgroundColor": "#FEF5E4", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#6D6E6D", "size": "sm"}}}
**FOCUS ACTIVITIES** (5 MINUTES)

**^[DO:]({"style": {"text": {"color":"#e6b349"}}})** Have an adult offer you a **present**. Reject the present and push it away; “I don’t want it!” Have them try to give it to you again as they explain that it is for everyone. Refuse the present again and knock it to the floor. Have the adult try to give the present a third time. Have the adult ask the children if they would like it. Give it to them.

**^[DISCUSS:]({"style": {"text": {"color":"#e6b349"}}})**

- **What did you think when I refused the present?** (Accept all responses.)

**Teaching Point:** God gives us a special present every week—a whole day with Him: the Sabbath. Many people are too busy to enjoy His gift. It is as though they refuse it in the same way the present was rejected.

- **How do you think God feels about that?** (Disappointed, sad, rejected.)
- **How do you think God feels when we accept the gift of the Sabbath?** (Happy, loved, appreciated.)

**^[READ:]({"style": {"text": {"color":"#e6b349"}}})**  Let’s learn more about God’s special gift to us!

Read the **Big Idea** together: **God gave us the Sabbath, a very special day to spend with Him.**

{"style": {"block": {"backgroundColor": "#ECF0E8", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#6D6E6D", "size": "sm"}}}
**BIBLE STORY** (15 MINUTES)

**^[SHARE:]({"style": {"text": {"color":"#649b7c"}}})** **Pray, asking God to help the children understand today’s story. Then creatively share the Bible story** with a doughnut discussion.

{"style": {"block": {"backgroundColor": "#ECF6F2", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#6D6E6D", "size": "sm"}}}
**UNPACK THE STORY** (5 MINUTES)

**^[MIME:]({"style": {"text": {"color":"#00b7aa"}}})** Have the children stand up and spread out. Say, “Day 1, Sunday,” and invite them to mime something they do on the first day of the week. Select a child to repeat their mime for the group to guess. Quickly repeat this process for the next five days of Creation.

**^[DISCUSS (MISSION):]({"style": {"text": {"color":"#00b7aa"}}})**

- **What do most of the activities you mimed on the first six days have in common?** (They are mostly work or things we do in our spare time.)
- **How were your Sabbath mimes different?** (They are focused on God: worshiping and spending time with Him.)
- **How could you help someone else enjoy the Sabbath?** (Challenge the children to think of someone specific—a younger sibling, a shut-in, a sick church member, etc.) Encourage the children to put their plan into action and be prepared to report back next Sabbath.

{"style": {"block": {"backgroundColor": "#F2ECF0", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#6D6E6D", "size": "sm"}}}
**TIE IT ALL TOGETHER** (20 MINUTES)

**^[MEMORY VERSE:]({"style": {"text": {"color":"#a771b0"}}})** Give each child **Character Card 3: The Sabbath** to place in their container. Discuss the picture and recite the memory verse together. Then sing the longer **Scripture Song, “Exodus 20:8-11.”**

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/3-exodus-20-8-11.mp3] “Exodus 20:8-11” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singer","value":"David Williams"},{"key":"Cello","value":"Jared Chung"},{"key":"Guitarist","value":"Katie Strachan"}],"title": "Credits","copyright":"Copyright © 2025 General Conference Corporation of Seventh-day Adventists®"}

- **What’s special about the Sabbath day and why?** Invite children to stand and share their answer to this question with two other people. (Ring a bell after one minute to indicate they should swap partners, and then again after the second minute to bring the children back together.)

Suggest that the children underline or highlight the memory verse in their Bibles at home.

**^[MISSION STORY:]({"style": {"text": {"color":"#a771b0"}}})** Share the **Mission Story**, “Such a Little Thing,” and discuss the follow-up question.

**^[MISSION CHALLENGE:]({"style": {"text": {"color":"#a771b0"}}}) Look for an older person in your neighborhood who may not know the truth about God’s
love. Pray for a way to gently share next week’s Big Idea with them.**

**^[CRAFT:]({"style": {"text": {"color":"#a771b0"}}})** Show the children your completed **Sunset-to-Sunset craft**.

- **What does this help us to remember?** (That the Sabbath begins at sunset on Friday and ends at sunset on Saturday.)

If time allows, show the children several ways we can **find out what time the sun sets** (church bulletin, online, or on some calendars). Distribute a craft outline to each child and have them cut out and assemble the craft.

**^[SING:]({"style": {"text": {"color":"#a771b0"}}})** Bring children back together to sing the closing song, **“God Bless You.”** Remind the children that we’ll worship God and celebrate Sabbath together in heaven. Won’t that be awesome?!

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/god-bless-you.mp3] “God Bless You” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"David Williams, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Violin","value":"Tahlia Atcheson"},{"key":"Guitarist","value":"David Williams"},{"key":"Bass","value":"David Williams"},{"key":"Clicks","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2025 General Conference Corporation of Seventh-day Adventists®"}

**^[RESPOND:]({"style": {"text": {"color":"#a771b0"}}})** Our loving God has given us a very special invitation to spend time with Him today and every Sabbath.

- **Will you spend time with Him?** This afternoon, talk to God as if He is sitting right next to you in person.

**^[PRAY:]({"style": {"text": {"color":"#a771b0"}}})** Ask God to be with the children and their families and to help them always remember God’s special gift of the Sabbath.