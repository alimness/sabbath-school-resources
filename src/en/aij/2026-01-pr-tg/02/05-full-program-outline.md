---
title: Full Program Outline
markdownTitle: '^[Full Program Outline]({"style": {"text": {"color": "#F4793B"}}})'
subtitle: Week 2
---

{"style": {"text": {"color": "#F4793B"}}}
**Big Idea:** We are God’s special creation, made in His image.

{"style": {"block": {"backgroundColor": "#E4E4E3", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#6D6E6D", "size": "sm"}}}
**EARLY ARRIVALS** (10 MINUTES BEFORE SABBATH SCHOOL BEGINS)

**^[LISTEN:]({"style": {"text": {"color":"#4d4d4f"}}})** Play the song **“When God Speaks”** or another worship song of your choice quietly in the background.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/when-god-speaks.mp3] “When God Speaks” {"credits":[{"key":"Writer","value":"Braedan Entermann"},{"key":"Singers","value":"Alise Entermann, Braedan Entermann"},{"key":"Pianist","value":"Braedan Entermann"},{"key":"Guitarist","value":"Braedan Entermann"},{"key":"Producer","value":"Rhys Zacher"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

**^[CONNECT:]({"style": {"text": {"color":"#4d4d4f"}}})** Take time to talk to each child individually as they arrive. Let them know that you care about them and that you are glad they have come.

**^[DO:]({"style": {"text": {"color":"#4d4d4f"}}})** Invite children to pair up to create face profiles. One child can lie down on their side while another traces around their head with a pencil. Then swap.

{"style": {"block": {"backgroundColor": "#F9E7E3", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#6D6E6D", "size": "sm"}}}
**OPENING ACTIVITIES** (10 MINUTES)

**^[WELCOME:]({"style": {"text": {"color":"#d34d5e"}}})** Bring the children together and welcome them as a group. Show some profiles from early arrivals and ask them to guess which profile belongs to each person.

**^[SING:]({"style": {"text": {"color":"#d34d5e"}}})** Sing **“God Is Our Creator.”** Ask the children if they have anything special they would like the group to pray about. Sing the prayer song, **“Be Still, Be Silent.”**

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/god-is-our-creator.mp3] “God Is Our Creator” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"David Williams, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Cello","value":"Jared Chung"},{"key":"Guitarist","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/be-still-be-silent.mp3] “Be Still, Be Silent” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"David Williams, Johanna McKay, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Violin","value":"Tahlia Atcheson"},{"key":"Cello","value":"Jared Chung"},{"key":"Bass","value":"David Williams"},{"key":"Glockenspiel","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

**^[PRAY:]({"style": {"text": {"color":"#d34d5e"}}})** Invite several children to pray, remembering to pray for the special requests.

**^[BIBLE FOCUS:]({"style": {"text": {"color":"#d34d5e"}}})** Sing **“Books of the Bible”** song while referring to the list of the books in order. Tell the children that if they’d like to, they can also listen to this song at home. Share a **Bible Fast Fact:** There are 66 books of the Bible. The Old Testament has 39 books, and the New Testament has 27 books.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/books-of-the-bible.mp3] “Books of the Bible” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Johanna McKay, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Guitarist","value":"David Williams"},{"key":"Bass","value":"David Williams"},{"key":"Percussion","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2025 General Conference Corporation of Seventh-day Adventists®"}

**^[GIVE:]({"style": {"text": {"color":"#d34d5e"}}})** Have a child collect the offering as you all sing **“Thank You, Jesus.”** Later, after the money has been counted by an adult, invite a child to color the next section of your **Offering Chart** to show how much has been collected this quarter. Then share this week’s **Mission Fast Fact** about this quarter’s mission project. Say a short prayer for God to bless and take the offering to where it’s most needed.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/thank-you-jesus.mp3] “Thank You, Jesus” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"David Williams, Johanna McKay, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Guitarist","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

**^[SING:]({"style": {"text": {"color":"#d34d5e"}}})** Warmly welcome any visitor and sing the **“Visitor Song.”** Sing **“Happy Birthday”** to a child who has had a birthday this week, and pray for God to bless them.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/visitor-song.mp3] “Visitor Song” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singer","value":"Johanna McKay"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/birthday-song.mp3] “Happy Birthday” {"credits":[{"key":"Writer","value":"Janet Sage"},{"key":"Singer","value":"David Williams"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Guitarist","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2001 North American Division Corporation of Seventh-day Adventists. From Little Voices Praise Him, #39. Used by permission"}

{"style": {"block": {"backgroundColor": "#FEF5E4", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#6D6E6D", "size": "sm"}}}
**FOCUS ACTIVITIES** (5 MINUTES)

**^[DO:]({"style": {"text": {"color":"#e6b349"}}})** Distribute a **magnifying glass** to each child and give them several minutes to explore the details in the nature objects on your **Nature Table**. Encourage them to notice the closeup texture, color, shape, patterns, etc. in each natural object.

**^[VIEW:]({"style": {"text": {"color":"#e6b349"}}})** Then compare these natural objects with human-made materials by looking at the **Nature Closeup Pictures**. Talk about the beautiful colors and detailed patterns on the **peacock feathers** when they are magnified. Show a piece of smooth **paper** and have children feel it before showing the picture of magnified paper.

**^[DISCUSS:]({"style": {"text": {"color":"#e6b349"}}})**

- **What do you think the paper will look like when it’s magnified?** (Accept all answers, then show the picture.)
- **When you look at what God has created and compare it to what people have created, what do you notice? (God’s creation is intricate, beautifully detailed, and patterned—even when highly magnified. Man’s creation is** random, disorderly.)
- **What did this comparing activity tell you about God?** (He is a God of order, beauty, intricate design, and careful thought.)

**^[READ:]({"style": {"text": {"color":"#e6b349"}}})** Read the **Big Idea** together from the whiteboard/poster paper: **We are God’s very special creation, made in His image.**

{"style": {"block": {"backgroundColor": "#ECF0E8", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#6D6E6D", "size": "sm"}}}
**BIBLE STORY** (15 MINUTES)

**^[SHARE:]({"style": {"text": {"color":"#649b7c"}}})** **Pray, asking God to help the children understand today’s story. Then creatively share the Bible story** with the animal card activity illustration and discussion.

{"style": {"block": {"backgroundColor": "#ECF6F2", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#6D6E6D", "size": "sm"}}}
**UNPACK THE STORY** (5 MINUTES)

**^[CREATE:]({"style": {"text": {"color":"#00b7aa"}}})** To recognize our limitations when compared with God’s creative power, give each child a piece of **modeling clay** or **play-dough**. Give them just a minute to sculpt a person and try to bring it to life.

**^[DISCUSS:]({"style": {"text": {"color":"#00b7aa"}}})**

- **Why couldn’t you bring your person to life?** (We can’t create life, only God can!)
- **What does the Bible say about how we are made? Read Psalm 139:14.** (We are “fearfully and wonderfully made.”)
- **What does God think of us? Read the first half of Isaiah 43:4.** (Precious, honored, loved.)

**^[TEACHING POINT:]({"style": {"text": {"color":"#00b7aa"}}})** Each of us are a “fearfully and wonderfully made” miracle, for only God can create a living, breathing person. We are all precious to Him.

{"style": {"block": {"backgroundColor": "#F2ECF0", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#6D6E6D", "size": "sm"}}}
**TIE IT ALL TOGETHER** (20 MINUTES)

**^[MEMORY VERSE:]({"style": {"text": {"color":"#a771b0"}}})** Give each child **Character Card 2: Adam and Eve** to place in their container. Discuss the picture
and recite the memory verse together. Then sing the **Scripture Song, “Genesis 1:27.”**

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/2-genesis-1-27.mp3?updated] “Genesis 1:27” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Yelena Jovinov, David Williams"},{"key":"Cello","value":"Jared Chung"},{"key":"Guitarist","value":"Katie Strachan"}],"title": "Credits","copyright":"Copyright © 2025 General Conference Corporation of Seventh-day Adventists®"}

Invite the children to discuss the following questions in pairs:

- **What does it mean to be “made in God’s image”?**
- **How do you feel, knowing you are made in God’s image?**

If time allows, have the children say the **memory verse** in pairs, saying one word each. Say it again faster.

**^[NATURE LESSON:]({"style": {"text": {"color":"#a771b0"}}})** Read the **Nature Lesson**, “Eye See,” and engage the children in the discussion questions.

**^[DO:]({"style": {"text": {"color":"#a771b0"}}})** Give each child a small **sticky note** and have them write “Thank You” on it. Invite them to stick it beside something on the Creation poster that they would like to thank God for. Alternatively, they could stick it beside their favorite animal card.

**^[CRAFT:]({"style": {"text": {"color":"#a771b0"}}})** Distribute a **Creation Bookmark** craft outline to each child and have them make the bookmark.

While children complete their craft, play the song **“When God Speaks.”** Invite the children to sing along.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/when-god-speaks.mp3] “When God Speaks” {"credits":[{"key":"Writer","value":"Braedan Entermann"},{"key":"Singers","value":"Alise Entermann, Braedan Entermann"},{"key":"Pianist","value":"Braedan Entermann"},{"key":"Guitarist","value":"Braedan Entermann"},{"key":"Producer","value":"Rhys Zacher"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

Alternatively have the children use **play-dough** or **modeling clay** to **sculpt** something (object or scene) to represent each of the six days of Creation.

**^[SING:]({"style": {"text": {"color":"#a771b0"}}})** Bring children back together to sing the closing song, **“God Bless You.”**

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-pr-tg/assets/god-bless-you.mp3] “God Bless You” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"David Williams, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Violin","value":"Tahlia Atcheson"},{"key":"Guitarist","value":"David Williams"},{"key":"Bass","value":"David Williams"},{"key":"Clicks","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2025 General Conference Corporation of Seventh-day Adventists®"}

**^[MISSION CHALLENGE:]({"style": {"text": {"color":"#a771b0"}}}) Share God’s special gift of the Sabbath with someone new.** Spend a moment talking and praying together about whom the children could invite to Sabbath School, on a Sabbath walk in nature, or to family worship next Sabbath. The gift of Sabbath shouldn’t be kept to ourselves!

**^[PRAY:]({"style": {"text": {"color":"#a771b0"}}})** Invite the children to form a **circle** and thank God for the part of God’s creation they stuck their sticky note beside. As you pray the final prayer, ask God to be with the children and help them always remember that they have been created in God’s image and that He loves them very much.