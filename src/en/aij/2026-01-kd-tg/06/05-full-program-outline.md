---
title: Full Program Outline
markdownTitle: '^[Full Program Outline]({"style": {"text": {"color": "#F4793B"}}})'
subtitle: Week 6
---

{"style": {"text": {"color": "#F4793B"}}}
**Big Idea:** God is loving. I will want to obey Him.

{"style": {"block": {"backgroundColor": "#E4E4E3", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**EARLY ARRIVALS** (10 MINUTES BEFORE SABBATH SCHOOL BEGINS)

**^[CONNECT:]({"style": {"text": {"color":"#4d4d4f"}}})** **Warmly welcome** each child by name and take time to talk with them individually as they arrive. Talk about what brought them happiness this week.

**^[DO:]({"style": {"text": {"color":"#4d4d4f"}}})** Invite the children to create a **play-dough face** that shows how they are feeling today without telling you the emotion. Invite the children to guess each other’s feelings. Encourage and acknowledge all their feelings and share that the people in our story today felt lots of different feelings. Listen to **“It’s About Love”** in the background.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/its-about-love.mp3] “It’s About Love” {"credits":[{"key":"Writer","value":"Tammy Cinzio"},{"key":"Singers","value":"Johanna McKay, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Guitarist","value":"David Williams"},{"key":"Midi Strings","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2003 Tammy Cinzio"}

**^[SING:]({"style": {"text": {"color":"#4d4d4f"}}})** Sing **“Pack It Up”** as you transition into the program.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/pack-it-up.mp3] “Pack It Up” {"credits":[{"key":"Writer","value":"Coralie Fraser"},{"key":"Singer","value":"Johanna McKay"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Guitarist","value":"David Williams"},{"key":"Midi Strings","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2022 General Conference Corporation of Seventh-day Adventists®"}

{"style": {"block": {"backgroundColor": "#F9E7E3", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**WELCOME AND SONGS** (5 MINUTES)

**^[WELCOME:]({"style": {"text": {"color":"#D34D5E"}}})** Warmly welcome the children again to Sabbath School today and tell them how happy you are to see them.

**^[SING:]({"style": {"text": {"color":"#D34D5E"}}})** Sing the following songs with actions and props:

- **“Good Morning, Jesus”**
- **“God’s Big Family”**
- **“God Is Our Loving Father”**
- **“Happy Birthday”** for any child with a birthday this past week
- **“Visitor Song”** and acknowledgment

```=Praise Songs

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/good-morning-jesus.mp3] “Good Morning, Jesus” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Johanna McKay, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Guitarist","value":"David Williams"},{"key":"Percussion","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/gods-big-family.mp3] “God’s Big Family” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Johanna McKay, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Cello","value":"Jared Chung"},{"key":"Guitarist","value":"David Williams"},{"key":"Percussion","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2025 General Conference Corporation of Seventh-day Adventists®"}

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/god-is-our-loving-father.mp3] “God is Our Loving Father” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Johanna McKay, TVAC Choir"},{"key":"Guitarist","value":"David Williams"},{"key":"Ukulele","value":"David Williams"},{"key":"Percussion","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2025 General Conference Corporation of Seventh-day Adventists®"}

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/birthday-song.mp3] “Happy Birthday” {"credits":[{"key":"Writer","value":"Janet Sage"},{"key":"Singer","value":"David Williams"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Guitarist","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2001 North American Division Corporation of Seventh-day Adventists. From Little Voices Praise Him, #39. Used by permission"}

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/visitor-song.mp3] “Visitor Song” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singer","value":"Johanna McKay"}],"title": "Credits","copyright":"Copyright © 2025 General Conference Corporation of Seventh-day Adventists®"}

```

{"style": {"block": {"backgroundColor": "#FEF5E4", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**OFFERING, PRAYER, MEMORY VERSE** (6 MINUTES)

**^[SHARE:]({"style": {"text": {"color":"#E6B349"}}})** Share a **Mission Fast Fact** about where the offering is going this quarter.

**^[GIVE:]({"style": {"text": {"color":"#E6B349"}}})** Sing **“For Jesus”** and invite a child to collect the offering in your offering container. After the money has been counted by an adult, invite a child to color the next section of your **Offering Chart** to show how much has been collected this quarter.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/for-jesus.mp3] “For Jesus” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Johanna McKay, Yelena Jovinov, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Guitarist","value":"David Williams"},{"key":"Midi Strings","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

**^[PRAY:]({"style": {"text": {"color":"#E6B349"}}})** Sing **“Let’s Pray.”** Read **Zephaniah 3:17**, “The Lord will be happy with you. . . . He will sing and be joyful about you” (ICB). Invite the children to kneel as you pray and thank God for being with you and loving you always. Pray that God will use the offering to help others choose Him.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/lets-pray.mp3] “Let’s Pray” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Johanna McKay, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Piano","value":"Jared Chung"},{"key":"Glockenspiel","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

**^[SING and DISCUSS:]({"style": {"text": {"color":"#E6B349"}}})** Invite a child to hold the special class Bible. Sing **“The Greatest Treasure.”**

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/the-greatest-treasure.mp3] “The Greatest Treasure” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Johanna McKay, TVAC Choir"},{"key":"Guitarist","value":"David Williams"},{"key":"Whistling","value":"David Williams"},{"key":"Percussion","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

- **What can we do to show God how much we love Him?**

Explain that this week’s memory verse from the Bible gives us an answer to this question. Show the children how to find the memory verse, **John 14:23. Say it together**: “If anyone loves Me, he will keep My word.”

Listen to, and sing, the **John 14:23 Scripture Song**. Explain that God invites us to listen to His words in the Bible because He knows that we’ll be so much happier when we do.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/6-john-14-23.mp3] “John 14:23” {"credits":[{"key":"Writer","value":"Sean Nebblett"},{"key":"Singer","value":"Sean Nebblett"},{"key":"Piano","value":"Sean Nebblett"}],"title": "Credits","copyright":"Copyright © 2025 General Conference Corporation of Seventh-day Adventists®"}

{"style": {"block": {"backgroundColor": "#ECF0E8", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**SECRET SURPRISE BOX** (7 MINUTES)

**^[DO:]({"style": {"text": {"color":"#649B7C"}}})** Play your **bell** or **sound maker**. Invite a child to open the **Secret Surprise Box** and show the pictures of Children’s Faces. Place the faces in a line and ask:

**^[DISCUSS:]({"style": {"text": {"color":"#649B7C"}}})**

- **Which face do you think shows happiness? disappointment? surprise? confusion? anger?** (You may like to invite different children to stand by a photo to answer each question while the remaining children make the expression.)
- **Which faces show how our loving God makes us feel on the inside?**

The way we feel inside will often show on our faces and in our actions.

**^[SHARE:]({"style": {"text": {"color":"#649B7C"}}})** Adam and Eve’s family had **inside feelings** that showed on their **outside faces**. Those feelings made them make good choices and bad choices. Sometimes they obeyed God, and sometimes they didn’t.

Share the **Big Idea**. Today, we are learning that **God is loving. I will want to obey Him.**

{"style": {"block": {"backgroundColor": "#ECF6F2", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**TELL THE STORY** (10 MINUTES)

**^[SHARE:]({"style": {"text": {"color":"#00b7aa"}}})** Pray, asking God to help the children understand today’s story. Then share the story **“Cain and Abel”** in an interactive way using the storytelling suggestions and Program PowerPoint.

{"style": {"block": {"backgroundColor": "#F2ECF0", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**STORY EXPLORER BOX** (5 MINUTES)

**^[DO:]({"style": {"text": {"color":"#A771B0"}}})** Play your **bell** or **sound maker**. Invite a child to open the **Story Explorer Box** to reveal a **child’s hat** and an **adult’s shirt**.

Invite a child to pretend to be a grown-up (put on an adult shirt), while you pretend to be a child (wear child’s hat). Have the “grown-up” (child) ask you to make your bed. Respond in the following ways: obey mostly, obey happily, grumble while you obey, disobey. After each response, invite the other children to give you a “thumbs up” for obedience or a “thumbs down” for disobedience. Act out other scenarios, such as: take out the trash, help with the dishes, collect the vegetables from the garden, be kind to your sister.

Explain how it’s only when we have love in our hearts that we will want to obey His rules of love with a happy, willing heart.

**Teaching Point:** God will always let people make their own choices—because that is what love does. God is happy to see people choose His way and obey Him.

Ask the children to stand and repeat the **Big Idea** together: **God is loving. I will want to obey Him.**

{"style": {"block": {"backgroundColor": "#E0E4EB", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**NATURE/MISSION** (3 MINUTES)

**^[NATURE LESSON and NATURE COLLECTOR CARD:]({"style": {"text": {"color":"#215E9E"}}})** Share the Nature Lesson: **“A Lion’s Pride.”** Give each of the children **Nature Card 6** for them to place in their Nature Collector Bag.

{"style": {"block": {"backgroundColor": "#FEEDDE", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**CRAFT** (15 MINUTES)

**^[CRAFT:]({"style": {"text": {"color":"#F4793B"}}}) Craft Activity: Card.** Make a card to give to somebody. Ask the children to share the message of their card with their family and to give it to someone who might need this message.

(Messages could include one of the following: When I love God, I will want to obey Him. Jesus can help me with my feelings, so I don’t hurt others. God’s way is my way.)

**^[REVIEW:]({"style": {"text": {"color":"#F4793B"}}})** Show the children the following objects from today’s lesson: **children’s faces, altar and cross picture, male lion**. Then ask:

- **What big, important lesson from today’s story does each object remind you of?**

After each lesson is shared, invite everyone to clap in celebration of what God has taught you.

- **Children’s Faces**: We’ve learned that our outside faces show our inside feelings. Let’s choose to ask Jesus to help us control unhappy or angry feelings so we don’t hurt other people.
- **Fruit/vegetables and a toy lamb**: We can say, “God’s way is my way.”
- **Picture of male lion**: When we choose peace, we choose love. This is God’s way.

Ask the children to repeat the **Big Idea** with you: Today we’ve learned that **God is loving. I will want to obey Him.**

**^[MISSION CHALLENGE:]({"style": {"text": {"color":"#F4793B"}}}) Choose a helpful job to do every day for a week to practice doing something faithfully (e.g., water the plants, make your bed, set the table). You may like to mark a calendar square every day you do this.**

**^[SING:]({"style": {"text": {"color":"#F4793B"}}})** Sing **“Goodbye Song.”** Share that next week we will learn about Noah building a big boat.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/goodbye-song.mp3] “Goodbye Song” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Johanna McKay, TVAC Choir"},{"key":"Ukulele","value":"David Williams"},{"key":"Percussion","value":"David Williams"},{"key":"Bass","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2025 General Conference Corporation of Seventh-day Adventists®"}

**^[PRAY:]({"style": {"text": {"color":"#F4793B"}}})** Pray, thanking God for helping us to obey with love in our hearts. Ask God to help us to ask for His help to be strong and learn from our mistakes when we disobey.