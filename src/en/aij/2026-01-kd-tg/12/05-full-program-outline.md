---
title: Full Program Outline
markdownTitle: '^[Full Program Outline]({"style": {"text": {"color": "#F4793B"}}})'
subtitle: Week 12
---

{"style": {"text": {"color": "#F4793B"}}}
**Big Idea:** God is loving. He can help me to be a loyal friend.

{"style": {"block": {"backgroundColor": "#E4E4E3", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**EARLY ARRIVALS** (10 MINUTES BEFORE SABBATH SCHOOL BEGINS)

**^[CONNECT:]({"style": {"text": {"color":"#4d4d4f"}}})** **Warmly welcome** each child by name as they arrive. Talk with them about their friends and what is so special about having a friend.

**^[PLAY:]({"style": {"text": {"color":"#4d4d4f"}}})** Invite the children to a table where the **puzzle pieces** are spread out. Encourage them to work together and assemble all pieces together to make the picture. As the children work, talk about what it is like to have special friends that work together. Listen to **“It’s About Love”** in the background.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/its-about-love.mp3] “It’s About Love” {"credits":[{"key":"Writer","value":"Tammy Cinzio"},{"key":"Singers","value":"Johanna McKay, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Guitarist","value":"David Williams"},{"key":"Midi Strings","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2003 Tammy Cinzio"}

**^[SING:]({"style": {"text": {"color":"#4d4d4f"}}})** Transition into the Sabbath School program by singing **“Pack It Up.”**

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/pack-it-up.mp3] “Pack It Up” {"credits":[{"key":"Writer","value":"Coralie Fraser"},{"key":"Singer","value":"Johanna McKay"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Guitarist","value":"David Williams"},{"key":"Midi Strings","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2022 General Conference Corporation of Seventh-day Adventists®"}

{"style": {"block": {"backgroundColor": "#F9E7E3", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**WELCOME AND SONGS** (5 MINUTES)

**^[WELCOME:]({"style": {"text": {"color":"#D34D5E"}}})** Warmly welcome the children again to Sabbath School today and tell them how excited you are to see them!

**^[SING:]({"style": {"text": {"color":"#D34D5E"}}})** Sing the following songs with actions and props:

- **“A Big Hello”**
- **“God’s Big Family”**
- **“Missionary, Missionary”**
- **“Happy Birthday”** for any child with a birthday this past week
- **“Visitor Song”** and acknowledgment

```=Praise Songs

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/a-big-hello.mp3] “A Big Hello” {"credits":[{"key":"Writer","value":"Amy Cherry"},{"key":"Singers","value":"Johanna McKay, David Williams"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Percussion","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/gods-big-family.mp3] “God’s Big Family” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Johanna McKay, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Cello","value":"Jared Chung"},{"key":"Guitarist","value":"David Williams"},{"key":"Percussion","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2025 General Conference Corporation of Seventh-day Adventists®"}

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/missionary-missionary.mp3] “Missionary, Missionary” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Johanna McKay, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Violin","value":"Tahlia Atcheson"},{"key":"Guitarist","value":"David Williams"},{"key":"Percussion","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/birthday-song.mp3] “Happy Birthday” {"credits":[{"key":"Writer","value":"Janet Sage"},{"key":"Singer","value":"David Williams"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Guitarist","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2001 North American Division Corporation of Seventh-day Adventists. From Little Voices Praise Him, #39. Used by permission"}

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/visitor-song.mp3] “Visitor Song” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singer","value":"Johanna McKay"}],"title": "Credits","copyright":"Copyright © 2025 General Conference Corporation of Seventh-day Adventists®"}

```

{"style": {"block": {"backgroundColor": "#FEF5E4", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**OFFERING, PRAYER, MEMORY VERSE** (6 MINUTES)

**^[SHARE:]({"style": {"text": {"color":"#E6B349"}}})** Share a **Mission Fast Fact** about where the offering is going this quarter.

**^[GIVE:]({"style": {"text": {"color":"#E6B349"}}})** Sing **“For Jesus”** and invite a child to collect the offering in your offering container. After the money has been counted by an adult, invite a child to color the next section of your **Offering Chart** to show how much has been collected this quarter.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/for-jesus.mp3] “For Jesus” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Johanna McKay, Yelena Jovinov, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Guitarist","value":"David Williams"},{"key":"Midi Strings","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

**^[PRAY:]({"style": {"text": {"color":"#E6B349"}}})** Sing **“Let’s Pray.”** Pray, asking God to take the offering to be used where it is most needed. Also ask Him to help the children understand the special story and lesson you will learn about today.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/lets-pray.mp3] “Let’s Pray” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Johanna McKay, TVAC Choir"},{"key":"Pianist","value":"Johanna McKay"},{"key":"Piano","value":"Jared Chung"},{"key":"Glockenspiel","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

**^[SING and DISCUSS:]({"style": {"text": {"color":"#E6B349"}}})** Invite a child to hold the special class Bible. Sing **“The Greatest Treasure.”**

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/the-greatest-treasure.mp3] “The Greatest Treasure” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Johanna McKay, TVAC Choir"},{"key":"Guitarist","value":"David Williams"},{"key":"Whistling","value":"David Williams"},{"key":"Percussion","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2024 General Conference Corporation of Seventh-day Adventists®"}

- **What do you think makes a good friend?** (Accept ideas; discuss.)

Explain that the Bible gives us a short answer to this question. Show the children how to find the **memory verse, Proverbs 17:17. Say it together**: “A friend loves at all times.”

Listen to, and sing, the **Proverbs 17:17 Scripture Song**.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/12-proverbs-17-17.mp3] “Proverbs 17:17” {"credits":[{"key":"Writer","value":"Sean Nebblett"},{"key":"Singers","value":"Heaven Nebblett, Sean Nebblett, Vanessa Nebblett"}],"title": "Credits","copyright":"Copyright © 2025 General Conference Corporation of Seventh-day Adventists®"}

- **Have you ever heard the word “loyal” to describe a friend?** Let’s find out a little more about what it means to be loyal.

{"style": {"block": {"backgroundColor": "#ECF0E8", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**SECRET SURPRISE BOX** (7 MINUTES)

**^[DO:]({"style": {"text": {"color":"#649B7C"}}})** Play your **bell** or **sound maker**. Invite a child to open the **Secret Surprise Box** to reveal a large T-shirt. Invite two friends to get into the T-shirt together. Instruct them to try to find the sock and shoe hidden in the room before you can count to 10. (Invite the other children to count slowly with you.)

**^[DISCUSS:]({"style": {"text": {"color":"#649B7C"}}})** Ask the children in the T-shirt:

- **How well did you stick together to find the shoe?**
- **How well do a sock and shoe stick together when you wear them?**
- **How might sticking together in a T-shirt and a sock/shoe remind us of good friends?**

**^[SHARE:]({"style": {"text": {"color":"#649B7C"}}})** Good friends stick together. We call this being loyal to each other. Loyal friends are kind, loving, and “there for each other” no matter what. They are “forever friends.”

Our loving God is our loyal friend too. He is the most loyal friend we can have.

Invite the children to stand up and repeat the **Big Idea** after you: **God is loving. He can help me to be a loyal friend.**

Today’s Bible story is about how loyal Abram rescued his nephew and good friend Lot from trouble.

{"style": {"block": {"backgroundColor": "#ECF6F2", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**TELL THE STORY** (10 MINUTES)

**^[SHARE:]({"style": {"text": {"color":"#00b7aa"}}})** Share the story **“A Brave Rescue”** in an interactive and engaging way using the **storytelling suggestions** and **Program PowerPoint**.

{"style": {"block": {"backgroundColor": "#F2ECF0", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**STORY EXPLORER BOX** (5 MINUTES)

**^[DO:]({"style": {"text": {"color":"#A771B0"}}})** Play your **bell** or **sound maker** and invite a child to open the **Story Explorer Box** to reveal a water bottle, two glass jars, and some food coloring. Pour water into the two glass jars. Add a drop of food coloring into one jar for each answer to the following question:

- **What bad choices did Lot make in our story today?** (He moved close to Sodom, visited Sodom, stayed the night in Sodom, moved into Sodom, made friends with the people in Sodom.)

Compare the water in the two glasses and highlight how the water got darker and darker with each tiny drop of color.

- **Which jar reminds you that one bad choice can lead to another and another?** (The jar of colored water.)
- **Which one reminds you to stay loyal to God and your friends who love Him?** (The clear water jar.)

**Teaching Point:** The dark jar reminds us of what happens when we choose friends that are making the wrong choices. Like the clear jar, our big loving God can help us to see clearly and choose loyal friends.

Repeat the **Big Idea: God is loving. He can help me to be a loyal friend.**

{"style": {"block": {"backgroundColor": "#E0E4EB", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**NATURE/MISSION** (3 MINUTES)

**^[NATURE LESSON and NATURE COLLECTOR CARD:]({"style": {"text": {"color":"#215E9E"}}})** Share the **Nature Lesson**: “The Belding’s Ground Squirrel.” Give each child **Nature Card 12: Ground Squirrel** to place in their Nature Collector Bag.

{"style": {"block": {"backgroundColor": "#FEEDDE", "padding": {"top": "sm", "start": "sm", "end":"sm", "bottom": "sm"}}, "text": {"color": "#4d4d4f", "size": "sm"}}}
**CRAFT** (15 MINUTES)

**^[CRAFT:]({"style": {"text": {"color":"#F4793B"}}})** Tithe Envelope. Make and decorate a tithe envelope to remind the children about why it is important to give to God.

Encourage the children to use it to return their own tithe to God. Remind the children that tithe belongs to God, and we are simply returning it to Him since this is what He’s asked us to do.

**^[REVIEW:]({"style": {"text": {"color":"#F4793B"}}})** Ask:

- **What might you do this week to be a loyal friend?** Remember to ask God to help you!

Repeat the **Big Idea: God is loving. He can help me to be a loyal friend.**

**^[MISSION CHALLENGE:]({"style": {"text": {"color":"#F4793B"}}}) Plan to invite someone over for a meal next Sabbath. Plan the menu together. Think of ways you can make your guest feel special and loved by God.**

**^[SING:]({"style": {"text": {"color":"#F4793B"}}})** Sing **“Goodbye Song.”** Share that next week we will be learning about kind baby Isaac.

!a[https://sabbath-school-resources-assets.adventech.io/en/aij/2026-01-kd-tg/assets/goodbye-song.mp3] “Goodbye Song” {"credits":[{"key":"Writer","value":"Rosie Smith"},{"key":"Singers","value":"Johanna McKay, TVAC Choir"},{"key":"Ukulele","value":"David Williams"},{"key":"Percussion","value":"David Williams"},{"key":"Bass","value":"David Williams"}],"title": "Credits","copyright":"Copyright © 2025 General Conference Corporation of Seventh-day Adventists®"}

**^[PRAY:]({"style": {"text": {"color":"#F4793B"}}})** Invite the children to link arms if they’d like to invite God to help them to be a loyal friend.

Invite a child to pray. Pray also and ask God to help the children to be loyal friends. Thank Him for always being their special loyal friend no matter what.